  ...
  <div>
    ...
    <div id="raceDiv" v-if="currentRace.name">
      ...
      <div class="subrace" v-for="(subrace, index) in currentRace.subraces" :key="subrace.id">
        ...
        <ul>
          <li v-for="trait in currentRace.traits" :key="trait.name"><div v-if="typeof trait === 'string'" v-html="$options.filters.marked(trait)"></div><div v-else><strong>{{ trait.name }}</strong><span v-html="$options.filters.marked(trait.description)"></span></div></li>
          <li v-for="trait in subrace.traits" :key="trait.name"><div v-if="typeof trait === 'string'" v-html="$options.filters.marked(trait)"></div><div v-else><strong>{{ trait.name }}</strong><span v-html="$options.filters.marked(trait.description)"></span></div></li>
        </ul>
      </div>
    </div>
