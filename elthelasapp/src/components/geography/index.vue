...
<div class="col-sm-12">
  <h1>Geography</h1>
  <div class="btn-group">
    <input type="button" class="btn btn-primary" @click="showCities()" value="Cities" />
    <input type="button" class="btn btn-primary" @click="showContinents()" value="Continents and Oceans" />
    <input type="button" class="btn btn-primary" @click="showFeatures()" value="Geographical Features" />
    <input type="button" class="btn btn-primary" @click="showLandmarks()" value="Landmarks" />
    <input type="button" class="btn btn-primary" @click="showNations()" value="Nations" />
    <input type="button" class="btn btn-primary" @click="showAll()" value="Show All" />
    <input type="button" class="btn btn-secondary" @click="showWarMap()"  v-if="!warmap" value="Show Terrotories of Control" />
    <input type="button" class="btn btn-secondary" @click="showWarMap()"  v-if="warmap" value="Hide Terrotories of Control" />
  </div>
  <div v-if="warmap">
    <div style="clear:left;"><div style="width:20px; height: 20px; float: left; background-color: blue;margin: 0px 10px 10px 10px; clear: left;"></div> Alliance</div>
    <div style="clear:left;"><div style="width:20px; height: 20px; float: left; background-color: red;margin: 0px 10px 10px 10px; clear: left;"></div> Orcs</div>
    <div style="clear:left;"><div style="width:20px; height: 20px; float: left; background-color: orange;margin: 0px 10px 10px 10px; clear: left;"></div> Manius Aquila's Rebellion</div>
  </div>
  <div id="map"></div>
  <b-modal id="nationmodal" size="lg" @hide="hideModal" :title="nation.name" ok-only>
    <div>
      ...
      <ul><li v-for="city in nation.cities" v-bind:key="city"><router-link :to="'/geo/city/' + city">{{city}}</router-link></li></ul>
      <p>
        <strong>Major Landmarks: </strong>
      </p>
      <ul><li v-for="landmark in nation.landmarks" v-bind:key="landmark"><router-link :to="'/geo/landmark/' + landmark">{{landmark}}</router-link></li></ul>
      <p>
        <strong>Bordering Nations: </strong>
      </p>
      <ul>
        <li v-for="nat in nation.borders" v-bind:key="nat"><router-link :to="'/geo/nation/' + nat">{{nat}}</router-link></li>
      </ul>
      <p>
      ...
      <table class="table table-striped">
        ...
        <tbody>
          <tr v-for="pop in nation.populationDistribution" v-bind:key="pop.race">
            <td><router-link :to="'/options/races/' + pop.race" >{{pop.race}}</router-link></td>
            <td>{{(pop.proportion * nation.population)}}</td>
            <td>{{pop.proportion * 100}}%</td>
          </tr>
          <tr>
            <td>Other</td>
      ...
      <ul>
        <li v-for="god in nation.gods" v-bind:key="god"><router-link :to="'/ref/gods/' + god" >{{god}}</router-link></li>
      </ul>
  ...
  <b-modal id="citymodal" size="lg" @hide="hideModal" :title="city.name" ok-only>
    ...
    <table class="table table-striped" :class="city.map != undefined ? 'col-sm-6': ''">
      ...
      <tr v-if="city.factions != undefined">
        <th>Major Factions</th><td><span v-for="faction in city.factions" v-bind:key="faction"><router-link :to="'/ref/orgs/' + faction">{{faction}}</router-link></span></td>
      </tr>
      <tr>
        <th>Notable Minorities</th><td><span v-for="minority in city.minorities" v-bind:key="minority"><router-link :to="'/options/races/' + minority" >{{minority}}</router-link>, </span></td>
      </tr>
      <tr>
    ...
    <div v-for="landmark in city.landmarks" v-bind:key="landmark.name">
      <h4>{{landmark.name}}</h4>
