<template>
  <div class="col-sm-12">
    ...
    <b-table show-empty
      ...
      <template v-slot:cell(name)="row"><a href="#" @click.stop="addToEncounter(row.item, row.index, $event.target)">{{ row.value }}</a> <button class="btn btn-sm btn-danger" @click="del(row.item)" v-if="isAdmin">X</button></template>
    </b-table>
