...
export default {
  computed: {
    ...
    hitdie () {
      if (this.character.size === "tiny") {
        return 4;
      } else if (this.character.size === "small") {
        return 6;
      } else if (this.character.size === "medium") {
        return 8;
    ...
    challengeRating () {
      ...
      if (hp < 7) {
        ...
        hpcr = Math.ceil((hp - 70) / 15);
      }
      var ac = this.accalc();
      var accr = Math.floor((ac - 11) * 1.34);
      var atts = this.character.attacks.reduce((a, b) => {
        return this.getStatMod(b.stat) + b.bonus + a;
      }, 0) / this.character.attacks.length;
      var dams = this.character.attacks.reduce((a, b) => {
        var total = 0;
        if (droll.validate(b.damage)) {
          total += droll.parse(b.damage).avgResult;
        } if (droll.validate(b.damage2)) {
          total += droll.parse(b.damage2).avgResult;
        }
        return a + total + Number(b.damagebonus) + b.addstat * this.getStatMod(b.stat);
      }, 0) / this.character.attacks.length;
      console.log(atts);
      console.log(dams);
      var attcr = Math.max(Math.ceil((atts) / 2), 0);
      var damcr = (dams - 10) / 5;
      var savecr = 0;

      return ((hpcr + accr) / 2 + (attcr + damcr + savecr) / 3) / 2;
    }
  },
  data () {
    return {
  ...
  methods: {
    ...
    profbonus() {
      return Math.ceil(this.challengeRating / 4) + 1;
    },
    getSkillMod(skill) {
    ...
    getHPTotal() {
      ...
      this.character.charclasses.forEach((cc) => {
        total += Math.floor(Number(cc.level) * (this.hitdie / 2 + this.getStatMod(2) + 0.5));
      });
