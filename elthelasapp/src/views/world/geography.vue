<template>
  <div class="col-sm-12">
    ...
    <modal id="nationmodal" :modalProps="modalProps.nation">
      <div>
        <div data-ng-if="nation.map != undefined" style="float:right; max-width: 40%;">
          <img :src="nation.map" :alt="nation.name" style="max-width:100%;" />
        </div>
        <p>
          <strong>Capitol: </strong> <router-link :to="'/world/geography/city/' + nation.capitol">{{ nation.capitol }}</router-link>
        </p>
        <p>
          <strong>Major Cities: </strong>
        </p>
        <ul><li v-for="city in nation.cities" :key="city"><router-link :to="'/world/geography/city/' + city">{{ city }}</router-link></li></ul>
        <p>
          <strong>Major Landmarks: </strong>
        </p>
        <ul><li v-for="landmark in nation.landmarks" :key="landmark"><router-link :to="'/world/geography/landmark/' + landmark">{{ landmark }}</router-link></li></ul>
        <p>
          <strong>Bordering Nations: </strong>
        </p>
        <ul>
          <li v-for="nat in nation.borders" :key="nat"><router-link :to="'/world/geography/nation/' + nat">{{ nat }}</router-link></li>
        </ul>
        <p>
        ...
        <div v-html="marked.parse(nation.government)"></div>
        <h4>History</h4>
        <div v-html="marked.parse(nation.history)"></div>
        ...
        <div v-html="marked.parse(nation.worship)"></div>
        <h4>Cultural Notes</h4>
        <div v-html="marked.parse(nation.culture)"></div>
        <h4>Economics</h4>
        <div v-html="marked.parse(nation.economics)"></div>
        <h3>Laws Concerning Common Topics</h3>
        <h4>Slavery</h4>
        <div v-html="marked.parse(nation.slavery)"></div>
        <h4>Magic</h4>
        <div v-html="marked.parse(nation.magic)"></div>
        <h4>Property</h4>
        <div v-html="marked.parse(nation.property)"></div>
        <h4>Citizenship</h4>
        <div v-html="marked.parse(nation.citizenship)"></div>
      </div>
    </modal>
    <modal id="citymodal" :modalProps="modalProps.city">
      <div v-if="city.map != undefined" style="float:right; max-width: 40%;">
        <img :src="city.map" :alt="city.name" style="max-width:100%;" />
      </div>
      <h4>National Allegiance: <router-link :to="'/world/geography/nation/' + city.nation">{{ city.nation }}</router-link></h4>
      ...
      <div v-html="marked.parse(city.description)"></div>
      ...
      <div v-for="landmark in city.landmarks" :key="landmark.name">
        <h4>{{ landmark.name }}</h4>
        <div v-html="marked.parse(landmark.description)"></div>
      </div>
    </modal>
    <modal id="landmarkmodal" :modalProps="modalProps.landmark">
      <div>
        <div v-html="marked.parse(landmark.description)"></div>
        <ul v-if="landmark.contains != undefined && landmark.contains.length > 0">
          <li v-for="nation in landmark.contains" :key="nation"><router-link :to="'/world/geography/nation/' + nation">{{ nation }}</router-link></li>
        </ul>
        <div style="clear:both;">&nbsp;</div>
      </div>
    </modal>
    <modal id="featuremodal" :modalProps="modalProps.feature">
      <div>
        <div v-html="marked.parse(feature.description)"></div>
        <ul v-if="feature.contains != undefined && feature.contains.length > 0">
          <li v-for="nation in feature.contains" :key="nation"><router-link :to="'/world/geography/nation/' + nation">{{ nation }}</router-link></li>
        </ul>
        <div style="clear:both;">&nbsp;</div>
      </div>
    </modal>
    <modal id="continentmodal" :modalProps="modalProps.continent">
      <div class="modal-body">
        <div v-html="marked.parse(continent.description)"></div>
        <ul v-if="continent.contains != undefined && continent.contains.length > 0">
          <li v-for="nation in continent.contains" :key="nation"><router-link :to="'/world/geography/nation/' + nation">{{ nation }}</router-link></li>
        </ul>
        <div style="clear:both;">&nbsp;</div>
      </div>
    </modal>
