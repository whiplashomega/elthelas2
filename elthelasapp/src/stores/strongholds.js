...
export default {
  ...
  getters: {
    ...
    availableLaborers() {
      return (
        Number(this.current.laborers) -
        this.current.improvements.reduce((total, imp) => {
          return total + Number(imp.laborers);
        }, 0) -
        this.current.construction.reduce((total, imp) => {
    ...
    availableStaffBeds() {
      return (
        this.staffBeds -
        (this.current.staff.length + this.current.privateEmployees.length)
      );
    },
    bankRevenue() {
      ...
      if (bank) {
        return (
          Math.round(
            (this.current.treasury * 0.002 + this.taxRevenue * 0.1) *
              this.calcRevRatio(bank) *
              100
          ) / 100
        );
      }
      ...
      this.current.construction.forEach((imp) => {
        ...
        if (!imp.private && !imp.dmGift) {
          for (var key in imp.resourceCost) {
            resourcecosts[key] += Number(
              percentdone.mul(imp.resourceCost[key])
            );
          }
        }
      });
    ...
    calcGrossRevenue() {
      let rev = this.current.improvements.reduce(
        (total, imp) => {
    ...
    calcTotalRevenue() {
      ...
      for (let key in rev) {
        rev[key] =
          Math.round((rev[key] + Number(this.current.autoSell[key])) * 100) /
          100;
      }
    ...
    calculateRevenue() {
      return (improvement) => {
        ...
        for (let res in improvement.revenue) {
          ...
          if (
            naturecleric &&
            (improvement.id === "food-farm" || improvement.id === "winery")
          ) {
            revenue[res] *= 1.25;
          }
          ...
          if (
            earthcleric &&
            (improvement.id === "coal-mine" ||
              improvement.id === "iron-mine" ||
              improvement.id === "quarry")
          ) {
            revenue[res] *= 1.25;
          }
        }
    ...
    expenses() {
      return (
        Math.round(
          (this.totalSalary + this.buildingMaintenance + this.resourceCost) *
            100
        ) / 100
      );
    },
    ...
    forestedLand() {
      return (
        this.totalLand -
        (this.availableClearedLand + this.farmLand + this.urbanLand)
      );
    },
    gameDate() {
      return (
        this.current.gameMonth +
        "/" +
        this.current.gameDay +
        "/" +
        this.current.gameYear
      );
    },
    getPop() {
      let pop =
    ...
    headTax() {
      return (
        Math.round(
          this.current.laws.headTaxRate * this.getPop * this.taxEfficiency * 100
        ) / 100
      );
    },
    improvementRevenue() {
    ...
    incomeTax() {
      let publicEmployees =
        (this.current.laws.incomeTaxRate / 100) *
        this.totalSalary *
        this.taxEfficiency;
      let privateIncomes =
        this.current.privateEnterprise.reduce((total, imp) => {
          return total + imp.employs * 0.5;
        }, 0) +
        this.current.privateEmployees.reduce((total, st) => {
          return total + st.typicalSalary;
        }, 0);
      let privateEmployees =
        (this.current.laws.incomeTaxRate / 100) *
        privateIncomes *
        this.taxEfficiency;
    ...
    rents() {
      ...
      if (rentRate) {
        return (
          rentRate * 5 * (this.staffBeds - this.availableStaffBeds) +
          rentRate * Math.min(publicHousing, numberRenting) -
          (this.current.laws.conscription
            ? this.current.guards * 5 * rentRate
            : 0) +
          (this.current.laws.conscription ? this.current.guard * rentRate : 0)
        );
      } else {
    ...
    maxLaborers() {
      return (
        this.current.population.adults -
        (this.privateLaborers +
          Number(this.current.servants) +
          Number(this.current.guards))
      );
    },
    totalHousing() {
      return (
    ...
    propertyTax() {
      let privateEnterpriseValue = this.current.privateEnterprise.reduce(
        (total, imp) => {
      ...
      return (
        Math.round(
          this.current.laws.propertyTaxRate *
            privateEnterpriseValue *
            this.taxEfficiency
        ) / 10
      );
    },
    ...
    staffBeds() {
      return (
        this.current.improvements.reduce((a, b) => {
        ...
        .reduce((a, b) => {
          ...
          let match = a.filter((st) => {
            if (!b.job.subtype) {
              ...
              return (
                b.job.name === st.job.name && b.job.subtype === st.job.subtype
              );
            }
          })[0];
          if (!needmatch) {
    ...
    taxEfficiency() {
      ...
      if (steward) {
        taxEfficiency = Math.min(
          1,
          (guardcount * 100 + constablecount * 500) / this.getPop
        );
      }
    ...
    totalSalary() {
      return (
        this.current.staff.reduce((a, b) => {
          return a + Number(b.salary);
        }, 0) +
        (this.current.laws.conscription
          ? this.current.guards * 0.5
          : this.current.guards * 2) +
        this.current.servants * 0.5 +
        this.current.laborers * 0.5
      );
    },
    totalStorage() {
    ...
    employablePeople() {
      return (
        ...
        this.current.staff.length +
        Number(this.current.privateEmployees.length)
      );
    },
    unemploymentRate() {
      return Math.max(
        this.current.population.adults
          ? Math.round(
              (1 -
                (this.totalEmployees + this.totalPrivateEmployed) /
                  this.employablePeople) *
                100
            )
          : 0,
        0
    ...
    unmetStaffNeed() {
      ...
      this.current.staff.forEach((st) => {
        ...
        needed.forEach((stn) => {
          if (
            ...
            st.job.subtype &&
            stn.name === st.job.name + " (" + st.job.subtype + ")"
          ) {
            index = needed.indexOf(stn);
            stn.num--;
          }
      ...
      let servants = needed.filter((a) => {
        return a.name === "Servant";
      })[0];
      if (servants) {
    ...
    unrest() {
      ...
      if (this.getPop > 0) {
        ...
        let foodSubsidiesModifier = this.getPop
          ? (Number(this.current.laws.foodSubsidies) / this.getPop) * 50
          : 0;
        unrest -= foodSubsidiesModifier;
        // people want a place to live. If there isn't enough housing, unrest increases
        let housingModifier = this.nonstaffPop
          ? Math.max(
              ((this.nonstaffPop - this.totalHousing) / this.nonstaffPop) * 100,
              0
            )
          : 0;
        unrest += housingModifier;
        if (this.current.laws.conscription) {
          let percentguards = Math.round(
            (this.current.guards / this.getPop) * 100
          );
    ...
    vassalTax() {
      ...
      if (taxcollector && Array.isArray(this.current.vassals)) {
        ...
        return (
          totalvassalincome * (Number(this.current.laws.vassalTaxRate) / 100)
        );
      } else {
        return 0;
      }
  ...
  actions: {
    ...
    saveNewStronghold() {
      axios
        .post("/strongholds?token=" + this.user.getUserInfo.token, {
          stronghold: { ...this.current, _id: undefined },
        })
        .then((res) => {
    ...
    addToTreasury({ changeby, record }) {
      ...
      this.current.transactionrecord.push({
        id: Date.now() + changeby + record,
        amount: changeby,
        description: record,
        balance: this.current.treasury,
        date: this.gameDate,
      });
      this.changeby = 0;
      this.record = "";
        ...
        improvement.staff.forEach((st) => {
          ...
          if (match && st.num > 0) {
            for (var x = 0; x < st.num; x++) {
              this.current.privateEmployees.push({
    ...
    buyResource({ type, amount }) {
      ...
      this.addToTreasury({
        changeby: -1 * Number(amount) * this.buyTable[type],
        record: "Purchase " + amount + " " + type,
      });
    },
  },
};
